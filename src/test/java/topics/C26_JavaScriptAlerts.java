...
public class C26_JavaScriptAlerts extends TestBase {
    /*
    Selenium Alerts, Selenium WebDriver ile çalışırken tarayıcıda karşılaşılan pop-up
     uyarıları veya onay kutularını yönetmek için kullanılan bir özelliktir. Bu uyarılar, kullanıcı
     etkileşimi gerektiren mesaj kutuları olabilir ve genellikle şu üç tipte karşımıza çıkar:

    Alert (Uyarı)
    Confirmation (Onay)
    Prompt (İstekte Bulunma)
