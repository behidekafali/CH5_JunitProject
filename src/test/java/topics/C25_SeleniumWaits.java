...
public class C25_SeleniumWaits extends TestBase {
...
Selenium waits (senkronizasyon): Kodlarımızı, internet ve bilgisayarın çalışmasını uyumlu hale getirmek için bazen kodlarımızı bekletmemiz gerekir.
    ...
    implicitlyWait: Selenium'dan gelir. Sayfanın yüklenmesi ve her bir WebElement'in locate edilmesi için beklenecek maksimum süreyi belirler.
    Esnektir. Esas olan beklemek değil, kodun çalışmasını sağlamaktır. Kod çalışırsa beklemeden devam eder, çalışmazsa maksimum süre
     doluncaya kadar bekler ve sonra hata mesajı verir (Exception).

    explicitlyWait: Selenium'dan gelir ve belirli bir işlem için beklenecek maksimum süreyi belirler. ExplicitlyWait kullanabilmek için wait
     objesi oluşturmamız gerekir.
